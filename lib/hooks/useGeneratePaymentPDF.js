  ...
  const generatePDF = async (payment, patient, returnBlob = false) => {
    ...
    return new Promise((resolve) => {
      ...
      reader.onloadend = () => {
        ...
        // Bloque de autofacturación: lo que el paciente teclea en el portal.
        ...
        doc.setFontSize(10);
        doc.setFont('helvetica', 'bold');
        doc.text('¿Necesitas factura? Autofactura tu pago:', 14, ay);
        doc.setFont('helvetica', 'normal');
        ay += 5;
        doc.text('Portal: https://autofacturacion.factura.com/697d1db74f793', 14, ay);
        ay += 5;
        doc.text(`Folio: ${folio}     Fecha: ${fd}/${fm}/${fy}     Total: ${money(totalConIva)}`, 14, ay);

        if (safePayment.notas) {
          doc.setFont("helvetica", "bold");
          doc.text('Notas:', 14, ay + 12);
          doc.setFont("helvetica", "normal");
          doc.text(String(safePayment.notas), 14, ay + 18);
        }
