...
export default function TreatmentDetailClient({ patientId, treatmentId }) {
  ...
  // Cargar paciente con token
  useEffect(() => {
    if (!patientId) return;
    if (patient?.id === Number(patientId)) return;
  ...
  if (!treatment) return <CircularProgress sx={{ mt: 4 }} />;
            ...
            {docsOfType.length > 0 ? (
              <Box display="flex" flexWrap="wrap" gap={1}>
                {docsOfType.map(doc => {
                  ...
                  return (
                    <Box key={doc.id} position="relative" width="100%">
                      {isPdf ? (
                        <Box
                          onClick={() => setPreviewFile({ preview: doc.file_url, type: 'application/pdf', name: doc.id })}
                      ...
                      <IconButton
                        ...
                        color="error"
                        sx={{ position: 'absolute', top: 0, right: 0, backgroundColor: 'rgba(255,255,255,0.7)', '&:hover': { backgroundColor: 'rgba(255,255,255,1)' } }}
                      >
                        <DeleteForeverIcon />
                      </IconButton>
                    </Box>
    ...
    <div className="px-10 py-10">
      <SectionTitle
        ...
        breadcrumbs={[
          ...
          { label: patient?.nombre || '', href: `/pacientes/${patientId}` },
          { label: 'Tratamientos', href: `/pacientes/${patientId}/tratamientos` },
        ]}
      />
      ...
      <Accordion
        sx={{ backgroundColor: '#F0F4FF', border: '2px solid transparent', '&:hover': { border: '2px solid #E8EFFF' } }}
      >
        ...
        <AccordionDetails>
          <Box display="flex" gap={2} sx={{ width: '100%', flexWrap: { xs: 'wrap', lg: 'nowrap' } }}>
            {DOCUMENT_TYPES.map(({ type, label }) => renderCard(type, label))}
          </Box>
        </AccordionDetails>
      </Accordion>

      <TreatmentDetailEvidences patientId={patientId} treatmentId={treatmentId} />
